...
Six tools, with deliberately non-overlapping jobs — ls finds files by name,
grep finds them by content, read views one, write creates or replaces one, edit
changes part of one, and bash does everything else. cici_101's TextEditorTool
folded view and create into the editor because it was the only tool in the
notebook; keeping that here would give the model two ways to do the same thing,
which is a reliable way to make it pick the wrong one.
...
def default_registry(workspace=None):
    ...
    Registration order is the order the model sees the tools in. Locating tools
    first, then read, then the mutating ones, follows the order a task actually
    goes in: find the file, look at it, then change it.
    """
    ...
    reg.register(GrepTool(workspace))
